					...
					<div className="grid gap-6 md:grid-cols-2">
						{data.chaincodes.map((chaincode) => (
							<Card
								...
								onClick={() => navigate(`/sc/fabric/chaincodes/${chaincode.id}`)}
							>
								<CardHeader>
									...
									<CardDescription>
										Network: {chaincode.network_name ?? chaincode.network_id ?? 'N/A'}<br />
										Platform: {chaincode.network_platform ?? 'N/A'}
									</CardDescription>
								</CardHeader>
								<CardContent>
									<div className="space-y-2">
										<p className="text-sm text-muted-foreground">Created <TimeAgo date={chaincode.created_at} /></p>
									</div>
								</CardContent>
							</Card>
						))}
