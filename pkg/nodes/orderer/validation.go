...
// Validate validates the StartOrdererOpts structure
func (opts *StartOrdererOpts) Validate() error {
	var errors []string
	...
	// Validate admin listen address
	if err := validateAddress(opts.AdminListenAddress, "adminListenAddress"); err != nil {
		errors = append(errors, err.Error())
	}
	...
	if len(errors) > 0 {
		return fmt.Errorf("orderer validation failed:\n  - %s", strings.Join(errors, "\n  - "))
	}
...
func (opts *StartOrdererOpts) checkPortConflicts() error {
	ports := make(map[string]string)

	addresses := map[string]string{
		"listenAddress":           opts.ListenAddress,
		"operationsListenAddress": opts.OperationsListenAddress,
		"adminListenAddress":      opts.AdminListenAddress,
	}
